...
- **Logger API uses Level + Category**: The `Logger.Log` method signature is `Log(level Level, cat Category, message string)`. All callers (including `dualLog`, TUI update handlers, and `root.go` startup code) use the named constants `logger.LevelInfo`, `logger.LevelWarn`, `logger.LevelError` for levels and `logger.CatStartup`, `logger.CatAuto`, `logger.CatRefresh`, etc. for categories. There is no `logger.Info`, `logger.Startup`, or `logger.EventType` type. The `Entry` struct has `Level` and `Category` fields (not a `Type` field). Do not flag correct 3-argument `Log` calls as compilation errors.
